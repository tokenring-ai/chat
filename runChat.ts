...
export default async function runChat({
  ...
  const client = await backoff(
    {times, interval: 1000, multiplier: 2},
    ({attempt}) => {
      if (attempt > 1) {
        agent.setCurrentActivity(
          `Chat model ${model} is not responding, retry ${attempt}/${times}`,
        );
      }
  ...
  for (const [, tool] of chatService.getAvailableToolEntries()) {
    ...
    if (tool?.toolDefinition?.adjustActivation) {
      const newEnabled = await tool.toolDefinition.adjustActivation(
        enabled,
        agent,
      );
  ...
  try {
        ...
        async stopWhen(options) {
          ...
          if (stepCount > 0) {
            const {compactionThreshold} =
              agent.getState(ChatServiceState).currentConfig.compaction;
            if (
              isThresholdReached(
                options.steps[stepCount - 1].usage,
                client,
                compactionThreshold,
              )
            ) {
    ...
    await agent
      ...
      ?.executeHooks(new AfterChatCompletion(response), agent);
    ...
    if (reachedCompactionThreshold) {
      if (
        ...
        if (
          !chatService.hasPendingCompaction(agent) &&
          !chatService.isCompactionInProgress(agent)
        ) {
          agent.infoMessage(
            "Context is getting long. Preparing a compaction summary...",
          );
          agent.setCurrentActivity("Preparing context compaction...");
          if (config.compaction.background) {
            void chatService
              .stageContextCompaction(config.compaction, agent)
              .catch((error) => {
                agent.errorMessage(
                  `Failed to prepare context compaction: ${error instanceof Error ? error.message : String(error)}`,
                );
              });
          } else {
        ...
        if (stopReason === "longContext" && appliedCompaction) {
          ...
          if (remainingSteps > 0) {
            agent.infoMessage(
              "Context compaction applied, and agent still has work to do. Continuing work...",
            );
    ...
    if (stopReason === "maxSteps") {
      if (agent.headless) {
        agent.infoMessage(
          "Agent stopped due to reaching the configured maxSteps",
        );
      } else {
        await agent.askForApproval({
          message:
            "Agent stopped due to reaching the configured maxSteps. Would you like to continue?",
        });
