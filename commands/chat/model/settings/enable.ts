...
function execute({ remainder, agent }: AgentCommandInputType<typeof inputSchema>): string {
  const args = remainder.split(/\s+/).filter(Boolean);
...
export default {
  name: "chat model settings enable",
...
/chat model settings enable reasoning
/chat model settings enable websearch temperature=0.7`,
